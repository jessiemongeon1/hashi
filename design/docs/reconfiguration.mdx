...
```mermaid
graph LR
    A[Start Reconfig] --> B[DKG or Key Rotation] --> C[End Reconfig] --> D[Guardian Committee Handoff]
```
...
```mermaid
graph LR
    A[Start Reconfig]:::active --> B[DKG or Key Rotation] --> C[End Reconfig] --> D[Guardian Committee Handoff]
...
```mermaid
graph LR
    A[Start Reconfig] --> B[DKG or Key Rotation]:::active --> C[End Reconfig] --> D[Guardian Committee Handoff]
    classDef active fill:#298DFF,stroke:#1759C4,color:#FFFFFF
```
...
```mermaid
graph LR
    A[Start Reconfig] --> B[DKG or Key Rotation] --> C[End Reconfig]:::active --> D[Guardian Committee Handoff]
    classDef active fill:#298DFF,stroke:#1759C4,color:#FFFFFF
```
...
```mermaid
graph LR
    A[Start Reconfig] --> B[DKG or Key Rotation] --> C[End Reconfig] --> D[Guardian Committee Handoff]:::active
    classDef active fill:#298DFF,stroke:#1759C4,color:#FFFFFF
```

After the Hashi epoch advances onchain, the guardian committee must also
transition to match the new on-chain epoch. A leader node drives this
handoff by coordinating the `SignCommitteeTransition` RPC among peers.

The `SignCommitteeTransition` RPC collects signatures from committee members
to authorize the guardian committee transition. Peers sign the transition
message using their BLS12-381 keys from a historical epoch rather than the
current one. Historical keys are required because the signing authority for
the transition belongs to the outgoing committee, which held the guardian
role during the previous epoch.

Once the leader gathers enough signatures to meet quorum, the guardian
committee updates to reflect the new on-chain epoch. This step ensures that
the guardian committee remains aligned with the Sui validator set and the
committed Hashi epoch at all times.
